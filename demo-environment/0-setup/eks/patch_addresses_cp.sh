...
# yq -i eval ".env.proxy_url = \"$PROXY_URL\"" kong-cp.yaml
yq -i eval ".env.admin_gui_url = \"$MANAGER_URL\"" kong-cp.yaml
yq -i eval ".env.admin_api_uri = \"$ADMIN_URL\"" kong-cp.yaml
yq -i eval ".env.portal_gui_host = \"$PORTAL_HOST:$PORTAL_PORT\"" kong-cp.yaml
yq -i eval ".env.portal_api_url = \"$PORTAL_ADMIN_URL\"" kong-cp.yaml
